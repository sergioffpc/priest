...
mod tests {
    ...
    fn encode_beq(rs1: u32, rs2: u32, imm: i16) -> u32 {
        ...
        imm_12 | imm_10_5 | (rs2 << 20) | (rs1 << 15) | (0b000 << 12) | imm_4_1 | imm_11 | 0b1100011
    }
    ...
    fn exec(inst: u32, hart: &mut Hart, bus: &mut Mmap) {
        Beq.call(inst, hart, bus)
            .expect("BEQ execution unexpectedly trapped");
    }
    ...
    fn beq_taken() {
        ...
        hart.set_xreg(1, 5);
        hart.set_xreg(2, 5);
        exec(encode_beq(1, 2, 16), &mut hart, &mut bus);
    ...
    fn beq_zero_offset() {
        ...
        hart.set_xreg(1, 7);
        hart.set_xreg(2, 7);
        exec(encode_beq(1, 2, 0), &mut hart, &mut bus);
        assert_eq!(hart.pc(), 0x3000);
    }

    #[test]
    fn beq_rd_x0_behavior() {
        let (mut hart, mut bus) = setup();
        hart.set_pc(0x4000);
        hart.set_xreg(1, 1);
        hart.set_xreg(2, 2);
        exec(encode_beq(1, 2, 8), &mut hart, &mut bus);
    ...
    fn beq_negative_offset() {
        ...
        hart.set_xreg(1, 3);
        hart.set_xreg(2, 3);
        exec(encode_beq(1, 2, -4), &mut hart, &mut bus);
        assert_eq!(hart.pc(), 0x5000 - 4);
    }
}
