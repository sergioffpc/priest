...
impl InstrExec for Bne {
    ...
    #[inline(always)]
    #[cfg_attr(feature = "trace", tracing::instrument(name = "BNE", skip_all, fields(rs1 = tracing::field::Empty, rs2 = tracing::field::Empty, imm = tracing::field::Empty)))]
...
mod tests {
    ...
    fn encode_bne(rs1: u32, rs2: u32, imm: i16) -> u32 {
        ...
        imm_12 | imm_10_5 | (rs2 << 20) | (rs1 << 15) | (0b001 << 12) | imm_4_1 | imm_11 | 0b1100011
    }
    ...
    fn exec(inst: u32, hart: &mut Hart, bus: &mut Mmap) {
        Bne.call(inst, hart, bus)
            .expect("BNE execution unexpectedly trapped");
    }
    ...
    fn bne_taken() {
        ...
        hart.set_xreg(1, 5);
        hart.set_xreg(2, 10);
        exec(encode_bne(1, 2, 16), &mut hart, &mut bus);
    ...
    fn bne_zero_offset() {
        ...
        hart.set_xreg(1, 7);
        hart.set_xreg(2, 10);
        exec(encode_bne(1, 2, 0), &mut hart, &mut bus);
    ...
    fn bne_negative_offset() {
        ...
        hart.set_xreg(1, 10);
        hart.set_xreg(2, 5);
        exec(encode_bne(1, 2, -8), &mut hart, &mut bus);
        assert_eq!(hart.pc(), 0x4000 - 8);
    }

    #[test]
    fn bne_equal_offset_zero() {
        let (mut hart, mut bus) = setup();
        hart.set_pc(0x5000);
        hart.set_xreg(1, 3);
        hart.set_xreg(2, 3);
