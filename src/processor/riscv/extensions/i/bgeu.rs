...
impl InstrExec for Bgeu {
    ...
    #[inline(always)]
    #[cfg_attr(feature = "trace", tracing::instrument(name = "BGEU", skip_all, fields(rs1 = tracing::field::Empty, rs2 = tracing::field::Empty, imm = tracing::field::Empty)))]
...
mod tests {
    ...
    fn encode_bgeu(rs1: u32, rs2: u32, imm: i16) -> u32 {
        ...
        imm_12 | imm_10_5 | (rs2 << 20) | (rs1 << 15) | (0b111 << 12) | imm_4_1 | imm_11 | 0b1100011
    }
    ...
    fn exec(inst: u32, hart: &mut Hart, bus: &mut Mmap) {
        Bgeu.call(inst, hart, bus)
            .expect("BGEU execution unexpectedly trapped");
    }
    ...
    fn bgeu_taken() {
        ...
        hart.set_xreg(1, 10);
        hart.set_xreg(2, 5);
        exec(encode_bgeu(1, 2, 16), &mut hart, &mut bus);
    ...
    fn bgeu_equal() {
        ...
        hart.set_xreg(1, 7);
        hart.set_xreg(2, 7);
        exec(encode_bgeu(1, 2, 12), &mut hart, &mut bus);
    ...
    fn bgeu_negative_offset() {
        ...
        hart.set_xreg(1, 5);
        hart.set_xreg(2, 2);
        exec(encode_bgeu(1, 2, -8), &mut hart, &mut bus);
    ...
    fn bgeu_zero_offset() {
        ...
        hart.set_xreg(1, 3);
        hart.set_xreg(2, 3);
        exec(encode_bgeu(1, 2, 0), &mut hart, &mut bus);
