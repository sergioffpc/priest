...
impl InstrExec for Blt {
    ...
    #[inline(always)]
    #[cfg_attr(feature = "trace", tracing::instrument(name = "BLT", skip_all, fields(rs1 = tracing::field::Empty, rs2 = tracing::field::Empty, imm = tracing::field::Empty)))]
...
mod tests {
    ...
    fn encode_blt(rs1: u32, rs2: u32, imm: i16) -> u32 {
        ...
        imm_12 | imm_10_5 | (rs2 << 20) | (rs1 << 15) | (0b100 << 12) | imm_4_1 | imm_11 | 0b1100011
    }
    ...
    fn exec(inst: u32, hart: &mut Hart, bus: &mut Mmap) {
        Blt.call(inst, hart, bus)
            .expect("BLT execution unexpectedly trapped");
    }
    ...
    fn blt_taken() {
        ...
        hart.set_xreg(1, 3);
        hart.set_xreg(2, 5);
        exec(encode_blt(1, 2, 16), &mut hart, &mut bus);
    ...
    fn blt_equal() {
        ...
        hart.set_xreg(1, 7);
        hart.set_xreg(2, 7);
        exec(encode_blt(1, 2, 12), &mut hart, &mut bus);
        assert_eq!(hart.pc(), 0x3004);
    }
    ...
    fn blt_negative_offset() {
        ...
        hart.set_xreg(1, 2);
        hart.set_xreg(2, 5);
        exec(encode_blt(1, 2, -8), &mut hart, &mut bus);
    ...
    fn blt_zero_offset() {
        ...
        hart.set_xreg(1, 5);
        hart.set_xreg(2, 5);
        exec(encode_blt(1, 2, 0), &mut hart, &mut bus);
